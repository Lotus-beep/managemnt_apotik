...
function Alatball() {
  ...
  const [alat, setalat] = useState(true);
  ...
  const barang = async()=>{
    const respon = await axios.get('http://localhost:8000/data_ALAT');
    const validasi = (respon.data);
    const reverse = validasi.reverse();
    setdata(reverse.filter((item)=> item.TIPE_barang === 'BALL'));
  }

  const percarian = async(e)=>{
    e.preventDefault();
    console.log(serch);
    const respon = await axios.post('http://localhost:8000/srcdata',{
    serc : serch
    })
  ...
  <div className="d-flex flex-row">
      ...
      <li className='nav-item dropdown'>
          ...
          {alat ? (
            <ul className="nav nav-pills flex-column mb-auto">
              ...
              <li className="nav-item">
                <a href="http://localhost:3000/Alat_ball" className="nav-link text-white active" aria-current="page">
        ...
        <li className="nav-item" style={{ zIndex: '0' }}>
          <a href="/Riwayat" className="nav-link text-white">
    ...
    <Beranda items={data} judul={"ALAT TYPE BALL"} style={{width:"100%"}}/> 
    </div>
  </div>
