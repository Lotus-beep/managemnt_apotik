...
function Category_strip() {
  ...
  const barang = async()=>{
    const respon = await axios.get('http://localhost:8000/data');
    const validasi = (respon.data);
    const reverse = validasi.reverse();
    setdata(reverse.filter((item)=> item.TIPE_barang === 'STRIP'));
  }
  ...
  <div className="d-flex flex-row">
        ...
        <li className='nav-item dropdown'>
          ...
          {list ? (
            <ul className="nav nav-pills flex-column mb-auto">
              ...
              <li className="nav-item">
                <a href="http://localhost:3000/type_strip" className="nav-link text-white active" aria-current="page">
    ...
    <Beranda items={data} judul={"TYPE STRIP"} style={{width:"100%"}}/> 
    </div>
  </div>
