...
function Alatpcs() {
  ...
  const barang = async()=>{
    ...
    setdata(reverse.filter((item)=> item.TIPE_barang === 'PCS'));
  }
  ...
  <div className="d-flex flex-row">
    ...
    <input type="text" class="form-control" aria-label="Amount (to the nearest dollar)" value={serch} onChange={(e)=>setserch(e.target.value)}/>
      ...
      <li className='nav-item dropdown'>
          ...
          {alat ? (
            <ul className="nav nav-pills flex-column mb-auto">
              <li className="nav-item">
                <a href="http://localhost:3000/Alat_pcs" className="nav-link text-white active" aria-current="page">
    ...
    <Beranda items={data} judul={"ALAT TYPE PCS"} style={{width:"100%"}}/> 
    </div>
  </div>
